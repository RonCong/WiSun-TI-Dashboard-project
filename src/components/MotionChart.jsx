...
function MotionChart() {
  ...
  useEffect(() => {
    ...
    fetch(`${urlString}/api/since?t=${formattedDate}&r=motionReading`)
      ...
      .then((data) => {
        ...
        let totalSensorDataLength = data?.motionReading?.length;
        ...
        for (var x = 0; x < totalSensorDataLength; x++) {
          let currentCodename = convertRawNodeToCodename(data?.motionReading[x]?.sensor)
          ...
          if ((x + 1) != totalSensorDataLength) {
            currentCodename = convertRawNodeToCodename(data?.motionReading[x+1]?.sensor)
            if (!sensorNames.includes(currentCodename)) {
              sensorDataLengths.push(currentDataLength)
              currentDataLength = 0
            }
          }
          else { //This is for the end of the motionReadings array, as the next element would not exist
            sensorDataLengths.push(currentDataLength)
            currentDataLength = 0
          }
        }
        ...
        for (var y = 0; y < sensorNames.length; y++) {
          ...
          for (var z = 0; z < sensorDataLengths[y]; z++) {
            let dataPoints = []
            //let numberDate = dateToNumber(data?.motionReading[curGlobalIndex]?.time)
            dataPoints.push(data?.motionReading[curGlobalIndex]?.time) //x (date(string) converted to number)
            dataPoints.push(data?.motionReading[curGlobalIndex]?.value) //y
  ...
  return (
    <>
      <div id="MotionChart" style={{ width: '65vw', height: '80vh', fontFamily: "'Raleway', sans-serif" }}></div>
    </>
  )
}
